...
- Выполняет результат компиляции TypeScript, предоставленный хостом; исходные `.ts`/`.mts`/`.cts` по умолчанию отклоняются, а legacy-стирание типов доступно только как явный режим миграции.
...
- TypeScript: по умолчанию принимается только скомпилированный результат; исходный TypeScript возвращает `ERR_AUTOJS6_TYPESCRIPT_COMPILER_REQUIRED`.
