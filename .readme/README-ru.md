...
* `Новое` Фатальная ошибка Node.js в слоте среды выполнения (нехватка памяти кучи JavaScript, сбой внутренней проверки) теперь оставляет собственный диагностический отчёт Node в кэше плагина (crash/<executionId>.json, записывается перед abort; обработчик сигналов не устанавливается): результат сбоя содержит событие, показатели кучи, стек JS и путь к отчёту в slotExit.crash и в сообщении об ошибке (например CRASH_NATIVE (SIGABRT; see the logcat tombstone; Node.js fatal error: Allocation failed - JavaScript heap out of memory; JS heap 61.9 MB used of 64.0 MB limit; report .../crash/<executionId>.json)), а getRuntimeInfo отдаёт новейший отчёт как lastCrash даже после перезапуска диспетчера; новая опция запроса maxOldGenerationSizeMb ограничивает кучу JS одного выполнения (старое поколение V8, в МБ), и именно так регрессионный тест воспроизводит этот сбой
* `Исправление` Исправлено падение слота среды выполнения из-за нехватки памяти кучи V8 (сообщалось как CRASH_NATIVE / SIGABRT) при require большого модуля CommonJS: маскирование комментариев и строк в загрузчике добавляло по одному символу, поэтому модулю в 22 MiB требовалось около 700 MB кучи; теперь маска строится из непрерывных фрагментов с тем же результатом (около 44 MB), а сканирование import / export в CommonJS переходит между кандидатами вместо проверки каждого символа
...
* `Улучшение` Тела запросов мостового fetch и сообщения WebSocket больше 256 КиБ теперь передаются хосту как файловый дескриптор только для чтения (bodyTransport / messageTransport "pfd") вместо встроенного base64 JSON на хостах, объявляющих bridgeRequestBinaryTransport=pfd (ветка хоста node-m20-2-binder-body-pfd), так что отправку ограничивают только политика запросов хоста (64 МиБ) и maxMessageBytes, а не размер транзакции Binder
* `Улучшение` Ответы контролируемого fetch теперь передают итоговый URL провайдера через Response.url (в ResponseInit нет url, поэтому раньше значение всегда было пустой строкой)
* `Улучшение` Java-взаимодействие теперь целиком передаётся декларативному белому списку хоста (класс → конструкторы, статические и экземплярные методы, поля; хост выполняет их через рефлексию с JSON-примитивами и дескрипторами объектов), среда больше не держит собственную таблицу классов, а читает опубликованную хостом как java.policy, добавлены getStatic() / describe(), а исключения самого члена сообщаются как ERR_AUTOJS6_JAVA_CALL_FAILED
* `Улучшение` Приоритет npm над модулями среды теперь распространяется только на модули, замещающие настоящие npm-пакеты (axios, colors, mime, nanoid, opencc, undici): одноимённый пакет в node_modules больше не подменяет фасады java, fetch, websocket, device и другие фасады AutoJs6, require.resolve следует тому же правилу, что и require, любое имя модуля среды можно импортировать из ESM, autojs6:profile публикует правило как moduleResolutionProfile, а прокси Rhino Packages получил getStatic() / describe()
* `Улучшение` Если процесс слота среды выполнения погибает во время выполнения скрипта (убит low-memory killer, нативный сбой, прямой kill), результат сбоя теперь сообщает системную запись о завершении вместо голого DeadObjectException: сообщение выглядит, например, как LOW_MEMORY (killed by the system low-memory killer; rss 2.6 GB) или CRASH_NATIVE (SIGABRT; see the logcat tombstone), результат содержит slotExit, а getRuntimeInfo содержит lastSlotExit (Android 11+; более старые системы сообщают, что причина недоступна)
