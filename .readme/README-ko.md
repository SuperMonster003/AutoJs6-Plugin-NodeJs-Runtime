...
- 호스트가 제공한 TypeScript 출력을 실행하고 실행 중 생성된 project `.ts`/`.mts`/`.cts`에 provider-v3 compilation을 요청할 수 있습니다. direct raw dispatch는 계속 fail-closed이며 legacy 타입 제거는 마이그레이션용 명시적 옵션입니다.
