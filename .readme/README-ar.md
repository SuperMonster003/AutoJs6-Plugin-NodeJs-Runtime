...
- يشغّل ناتج مترجم TypeScript الذي يقدمه المضيف ويدعم عبر provider v3 ترجمة ملفات `.ts`/`.mts`/`.cts` المنشأة أثناء التشغيل عند الطلب; أما الإرسال الخام المباشر فيظل fail-closed ومحو الأنواع legacy خيار ترحيل صريح فقط.
