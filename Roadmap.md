...
- [ ] **M11.5 发布 1.2.0 (对外动作, 需用户确认)**: ① 确认 `VERSION_NAME=1.2.0`、`VERSION_BUILD` = `git rev-list --count HEAD`; ② `:app:appendDigestToReleasedFiles` 产出 4 个带 CRC32 的已签名 APK, 逐个 `zipalign -c -P 16 -v 4` 校验并安装冒烟 (arm64 真机 + x86_64 模拟器); ③ 推送 master (对 origin/master 为 fast-forward), 打 `v1.1.0` (指向 M2~M4 收尾提交 03734c9 或 2e762b5, 二选一记录) 与 `v1.2.0` 两个 tag, 创建 GitHub Release (说明取 CHANGELOG-zh-Hans/en 对应节); ④ feature 分支 `feature/mediainfo-snapshot-v2-contract` 在 M11.2 吸收后删除本地与远端。Check: `gh release view v1.2.0` 列出 4 个 APK 且 CRC32 与文件一致; `git status --short` 为空; `origin/master == master`。 — 2026-09-08 本地发布准备完成, 待用户批准对外动作: build 105 四份签名 APK 已收集至 app/releases/1.2.0/, CRC32 为 arm64 08C05B3A / armv7 0D180A10 / x86_64 BC90135B / universal 89E6B200。修复收集任务: 依赖 assembleRelease、读取实际 outputs/apk/release、验证签名/恰好四包/ABI 库集合/ELF 并按版本收集, 只清理本版本过期产物。新增 nodeAndroidTestBuildType=release 与测试 APK 独立 Kotlin 运行库; 解决测试依赖被 AGP 按应用依赖扣除而 R8 又裁剪掉类的问题, 元数据测试改读已安装资源。四包逐个通过 apksigner、zipalign -c -P 16 -v 4、CRC32/版本/许可证核验; arm64 小米 API35、armv7 Sony G8441 API28、x86_64 16KB API36 AVD、universal 小米四次安装各通过 SimpleRun/NpmEcosystem/Info/Manifest 4/4, 合计 16/16。中英 Release 正文已从生成的 CHANGELOG 提取至 build/m11/release-notes-v1.2.0.md。远端 master 已确认是本地祖先; 原 feature 分支本地与远端均不存在, 无需删除。历史 v1.1.0 标签的两个候选 03734c9 (build49) 与 2e762b5 (build51) 实际 VERSION_NAME 均为 1.1.3, 存在版本命名冲突, 本轮不自动补打; 将此差异交用户裁定。当前尚未 push/tag/创建 Release, 本项与 M11.3 保持未勾选。
