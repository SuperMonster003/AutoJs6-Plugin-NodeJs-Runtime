...
全局 `fetch`、`Request`、`Response`、`Headers`、`FormData`、`WebSocket` 使用 Node 自带的 Web API (网络实现为内置 Undici)。全局网络请求与 raw Node http/https 一样默认可用, 无需宿主 `network` 声明; 请求设 `rawNodeNetworkModulesEnabled=false` 会拒绝全局 fetch/WebSocket 的网络操作, 非联网的数据类仍可用。需要宿主代理、证书与网络策略时, 显式使用 `require("autojs6:fetch")` 或 `require("autojs6:websocket")`, 这两个桥模块仍独立检查宿主 `network` 能力。桥接版的超时上限、响应体大小、方法集合与 WebSocket 消息 / 队列尺寸由宿主网络提供者决定 (宿主 `ControlledFetchPolicy` / `ControlledWebSocketPolicy`: fetch 超时 ≤ 120 s、响应默认 32 MiB / 上限 64 MiB、仅 GET / POST; WebSocket 连接超时 ≤ 60 s、消息默认 64 KiB / 上限 1 MiB、队列默认 32 / 上限 128), 运行时只填 Node 风格默认值 (fetch 超时 30 s、WebSocket 连接超时 10 s、跟随重定向至多 20 次) 并把调用者选项原样传给宿主, `policy.limitsEnforcedBy` 为 `host_provider`。正文与消息经 Binder 以 base64 JSON 往返, 实测单次事务超过约 1 MB 即失败 (2 MiB 响应在宿主侧抛 TransactionTooLargeException, 脚本侧得 ERR_AUTOJS6_NETWORK_TIMEOUT; 2 MiB 发送在插件侧的 Binder 调用即抛出, 脚本侧得 ERR_AUTOJS6_BRIDGE_PROVIDER_FAILED), 更大的正文请使用全局原生 fetch / WebSocket。
