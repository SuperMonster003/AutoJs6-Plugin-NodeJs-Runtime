...
全局 `fetch`、`Request`、`Response`、`Headers`、`FormData`、`WebSocket` 使用 Node 自带的 Web API (网络实现为内置 Undici)。全局网络请求与 raw Node http/https 一样默认可用, 无需宿主 `network` 声明; 请求设 `rawNodeNetworkModulesEnabled=false` 会拒绝全局 fetch/WebSocket 的网络操作, 非联网的数据类仍可用。需要宿主代理、证书与网络策略时, 显式使用 `require("autojs6:fetch")` 或 `require("autojs6:websocket")`, 这两个桥模块仍独立检查宿主 `network` 能力。桥接版的超时上限、响应体大小、方法集合与 WebSocket 消息 / 队列尺寸由宿主网络提供者决定 (宿主 `ControlledFetchPolicy` / `ControlledWebSocketPolicy`: fetch 超时 ≤ 120 s、响应默认 32 MiB / 上限 64 MiB、仅 GET / POST; WebSocket 连接超时 ≤ 60 s、消息默认 64 KiB / 上限 1 MiB、队列默认 32 / 上限 128), 运行时只填 Node 风格默认值 (fetch 超时 30 s、WebSocket 连接超时 10 s、跟随重定向至多 20 次) 并把调用者选项原样传给宿主, `policy.limitsEnforcedBy` 为 `host_provider`。桥接 fetch 的响应体经 ParcelFileDescriptor 交付 (M20.2 第十三批: 请求带 `binary: true`, 支持的宿主把正文写入缓存文件并以 `bodyTransport: "pfd"` 附带描述符, 运行时 mmap 后挂为 Buffer; 空正文与旧宿主仍内联 base64), 响应体只受宿主 `maxResponseBytes` 约束; 插件到宿主方向 (fetch 请求正文、WebSocket 消息) 仍以 base64 JSON 经 Binder 同步往返, 单次事务约 1 MB (2 MiB 发送在插件侧的 Binder 调用即抛出, 脚本侧得 ERR_AUTOJS6_BRIDGE_PROVIDER_FAILED); 宿主回复超过 Binder 事务尺寸时 (如未带 binary 的旧插件收到 2 MiB 内联响应), 第十三批宿主改为立即回一条小体积 BRIDGE_PROVIDER_FAILED, 脚本不再等到超时。更大的上行正文请使用全局原生 fetch / WebSocket。
