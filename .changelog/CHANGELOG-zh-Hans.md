...
* `修复` OpenSSL STORE 密钥 URL 绕过文件系统限制的问题 (请通过 node:fs 读取密钥字节)
...
* `优化` Node 脚本支持协商式异步启动, 长驻执行释放 Binder 线程, 终态回调完成后回传工作区, 兼容旧同步宿主
* `依赖` 升级 Node.js 24.5.0 → 24.21.0, 三个 ABI 均使用自建 Android 源码产物
