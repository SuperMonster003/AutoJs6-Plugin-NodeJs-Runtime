...
- 执行宿主提供的 TypeScript 编译产物; raw `.ts`/`.mts`/`.cts` 默认 fail-closed, legacy 类型剥离仅供迁移期显式启用.
...
- TypeScript: 默认只接受编译产物; raw TypeScript 返回 `ERR_AUTOJS6_TYPESCRIPT_COMPILER_REQUIRED`.
